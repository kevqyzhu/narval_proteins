...
def grid_search(chem):
    ...
    # permutations_dicts[-1].update(params)
    # write_sh_script(chem, permutations_dicts[-1])
...
#SBATCH --job-name=test_{chem}_selfies
#SBATCH --gres=gpu:1
#SBATCH --mem=64G
...
cd ../{chem}
directory=models/{chem}_models_selfies_seq_{params['seq_length']}_numlayers_{params['n_layers']}_hidden_{params['n_hidden']}_dp_{params['drop_prob']}_lr_{params['lr']}
mkdir $directory
python3 ../../RNN/test.py --n_layers {params['n_layers']} --load_epoch {params['load_epoch']} --tokens ../../data/{chem}_tokens_selfies.txt --encoded ../../data/{chem}_encoded_selfies.pickle --save_dir $directory --seq_length {params['seq_length']} --n_epoch {params['n_epoch']} --n_hidden {params['n_hidden']} --drop_prob {params['drop_prob']} --lr {params['lr']} --batch_size {params['batch_size']}
        ''')
